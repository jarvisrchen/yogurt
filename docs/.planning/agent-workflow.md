...
jarvisrchen read Part 1 of lauren (poteto)'s pstack guide and asked what in it applies here.
...
Everything that is judgment stays with the agent or with jarvisrchen: what to build, whether a doc PR describes something installable at this tag, the release narrative, the manual-test clicks, and whether to merge at all.
...
Makes tmux a documented dev dependency in CONTRIBUTING.md; it is installed (3.5a) and is jarvisrchen's stated preference.
...
Never pulls the shared checkout; prints the pull command for jarvisrchen instead.
...
`just pr` never merges and `just land` always does, so the task prompt chooses; 13 of the last 20 PRs merged within three minutes of opening, three waited hours for jarvisrchen.
...
jarvisrchen's global Claude Code settings already set `attribution` to empty, so the hook is the backstop for other agents, not a duplicate.
Absorbs AGENTS.md line 50's enforcement half and line 57.

**B4. GitHub settings, one time** (now, minutes, jarvisrchen's call).
...
No ruleset on `main` (decision 1): jarvisrchen is the sole collaborator on a public repo, so nobody else can push there today, and a PR requirement would only constrain him.
...
Once `just start` exists an agent is never in the main checkout to begin with, the pre-commit hook (B3) stops the commit half of the failure, and the guard's remaining friction lands on jarvisrchen, who is the one who runs from `main`.
...
Keep the notes about jarvisrchen's preferences; those are not in the repo.
...
- A ruleset on `main` requiring a pull request, `gh pr merge --auto`, or required status checks. jarvisrchen is the sole collaborator, so a PR requirement only binds him; docs-only PRs never report a check, so a required check blocks a third of PRs forever; `land` waiting 1-3 minutes synchronously is simpler.
...
- The shared-checkout tree guard itself, and any persistent `YOGURT_OWN_MAIN` export. `just start` keeps agents out of the main checkout; the guard would only tax jarvisrchen, and a profile export would disable it for every agent spawned from that shell.
...
Fourteen of the questions this research raised have a defensible default or an answer from jarvisrchen, so they are decided here rather than left open.
...
| 10 | tmux as a documented dev dependency (A4) | yes | jarvisrchen asked for tmux on 2026-08-28 |
| 11 | Shared-checkout tree guard (B5) | do not build | with `just start` and the pre-commit hook an agent is never in the main checkout to begin with; the guard's remaining cost lands on jarvisrchen every time he runs from `main`. Revisit if an agent builds there after DX-3 lands |
...
| 1b | Ruleset requiring a PR on `main`? (B4) | no, per jarvisrchen | sole collaborator on a public repo; nobody else can push, and the rule would only bind him |

The one that needs jarvisrchen, because it is an admin action on his account and binds his own pushes:

1. Change the repo settings once: squash-only and delete branch on merge (B4).
jarvisrchen decided against a ruleset on `main`: he is the sole collaborator, so nobody else can push there, and a PR requirement would only constrain him.
