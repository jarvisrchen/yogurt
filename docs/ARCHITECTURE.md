...
| Auto-*starting* a recording on detection (MTG-11, 2026-09-01) | The title patterns are vendor UI strings that drift, so the first false positive would silently record a room. A privacy-first app does not get to make that mistake by default | Detect and prompt: a dismissable banner offering "Start recording", which runs the same create-then-`autoStart` path as "+ New meeting". The auto-*stop* half is kept, since stopping errs toward a call you restart rather than a room you did not know was taped |
