...
- `meetings.llm_model` (`"cli:claude"` / `"cli:cursor-agent"`) is accurate by construction - the active provider IS the CLI, not a fallback standing in for something else.
