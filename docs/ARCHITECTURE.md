...
graph LR
    ...
    DIRECT --> CLI["yogurt_llm::CliClient<br/>claude -p / cursor-agent -p / opencode run<br/>JSON or JSON-lines output [--model]"]
    CLI -.->|"spawns, scratch cwd,<br/>--restricted --effort low for claude,<br/>--trust --sandbox enabled for cursor-agent,<br/>--pure + OPENCODE_PERMISSION deny for opencode"| BIN["local claude / cursor-agent / opencode binary"]
```

`resolve` reaches a `CliClient` only when the active provider row is itself `cli`-adapter - the Settings page's "Claude Code (local CLI)" / "Cursor Agent (local CLI)" / "OpenCode (local CLI)" presets, cloned like any other provider. It calls `CliClient::locate(CliProgram::parse(row.model), row.cli_model)` directly. The program not being on `$PATH` is a hard `Err`, the same contract as an `http` provider whose key can't be read - never a silent `MockLlm` substitution. `providers.base_url` is unused for a `cli` row (empty string); `providers.model` is repurposed to hold the `CliProgram` id ("claude" | "cursor-agent" | "opencode") instead of a model name, and the Settings UI hides the BASE URL / API KEY fields entirely for it - just a one-line "runs `<program>` locally" note, a `Test` button that needs no key, and a MODEL field bound to the separate `providers.cli_model` column (the `--model` alias passed through, e.g. "haiku"/"sonnet"/"opus"/"fable" for claude; empty means "use the CLI's own default"). These are deliberately "simple AI calls" (enhance/chat extraction, not open-ended reasoning), which is why the smallest model and the lowest effort setting are the defaults.
...
- Each call is a fresh process; there is no session to resume, so the full message history is flattened into one prompt argument the same way `OpenAiCompatClient` re-sends the full history over HTTP - `-p <prompt>` for claude/cursor-agent, a trailing positional after every flag for `opencode run`.
- Meeting-transcript text is untrusted - it is whatever the other party on the call said - and it reaches the CLI as plain prompt text, not through a reviewed tool-use loop. `claude` runs with `--restricted --strict-mcp-config --disable-slash-commands --effort low` (plus `--model <alias>` when `cli_model` is set) and a throwaway scratch `cwd` so a transcript that reads like an instruction can't do anything but produce text. `cursor-agent` runs with `--trust --sandbox enabled` (plus `--model` when set) - `--trust` clears the "Workspace Trust Required" wall the CLI otherwise throws up for a fresh scratch `cwd`, and `--sandbox enabled` keeps command execution contained; deliberately not `--yolo`/`--force` (cursor-agent's broad auto-approve-commands flag), same reasoning as never passing `--dangerously-skip-permissions` to `claude`. All three programs' exact flag sets are verified against live binaries.

- **`opencode`'s containment is not expressible in argv, and the obvious flag is the wrong one.** `--auto` ("auto-approve permissions that are not explicitly denied (dangerous!)") is how a user normally makes `opencode run` non-interactive, and it is precisely what must never be passed here. Worse, a bare `opencode run` still loads the user's global config: verified live on a real machine, the model was offered `google-workspace_send_gmail_message`, `google-workspace_search_gmail_messages` and Drive writes - a working exfiltration path for a transcript that says "email yourself the following". `OPENCODE_CONFIG_CONTENT={}` does not close it; the global `mcp` block loads regardless. What does is the environment variable `OPENCODE_PERMISSION={"*":"deny"}`, which removes every tool from the model's schema rather than prompting about it. That is the right shape here anyway - enhance and chat are text-in/text-out extraction calls that have never needed a tool. Measured on a live run it takes the request from ~35k input tokens (every tool, skill and MCP server) to ~2.2k, and a prompt ordering the model to run `echo` comes back as a fabricated transcript in prose rather than a `tool_use` event: there is nothing left to call. `--pure` pairs with it to drop third-party plugins, which no permission entry covers.
- **`opencode` speaks JSON lines, not one JSON object.** `opencode run --format json` prints one event per line - the answer as `type: "text"` parts, thinking as `type: "reasoning"` (deliberately dropped; it is the model's scratchpad, not its reply), and a failure as a `type: "error"` line. So `interpret_output` dispatches on `CliProgram` instead of assuming the claude/cursor-agent single-object shape. Text is keyed by `part.id` and joined in first-appearance order: a live run emits exactly one text event holding the whole answer, and the keying is what keeps it correct if opencode ever streams a part as growing snapshots, where blind concatenation would repeat the prefix.
- Model catalogs come from the binary: `cursor-agent --list-models` prints `<id> - <label>` lines, `opencode models` prints bare `<provider>/<model>` lines (370 on a live account, spanning every provider the user has configured). One parser reads both - a ` - ` separator ends the id when present, the whole line is the id when it isn't, and a whitespace check drops headers and prose either way. `opencode`'s preset ships one static suggestion, `minimax-coding-plan/MiniMax-M3`, which is also its `default_cli_model`; Refresh is how the rest becomes visible.
...
- **Parse stdout before checking the exit code, not after.** This holds for all three, in both output formats: `opencode` exits 1 on an unroutable model having printed one `error` line and nothing on stderr. `claude --output-format json` still writes a structured, actionable result to stdout on many failures - caught live against a real "not logged in" account: exit status 1, empty stderr, and `{"is_error":true,"result":"Not logged in · Please run /login"}` on stdout. A first cut of `CliClient` checked `status.success()` first and only looked at stderr on failure, so the Settings `Test` button showed a useless "claude CLI exited with exit status: 1: " instead of the actual reason. `interpret_output` (pure, unit-tested with a synthetic `ExitStatus` - no process needed) now parses stdout as JSON first regardless of exit status, and only falls back to raw exit-status/stderr when stdout isn't parseable JSON at all (a crash before the CLI's own output formatting ever ran).
- `meetings.llm_model` (`"cli:claude"`, or `"cli:claude:haiku"` / `"cli:opencode:minimax-coding-plan/MiniMax-M3"` when a `--model` override is set) is accurate by construction - the active provider IS the CLI, not a fallback standing in for something else.
...
| Calendar (EventKit) or a system-audio heuristic as the meeting-detection signal (MTG-11, 2026-09-01) | Calendar needs a *new* TCC permission, fires on events you decline or skip, and misses every ad-hoc call. A system-audio heuristic means holding an `SCStream` open around the clock just to listen for speech - capturing audio while not recording, which is the exact thing the privacy constraint exists to prevent | On-screen window titles via `SCShareableContent` (`yogurt_audio::detect`): a read of already-authorized metadata under the Screen Recording grant loopback already requires, so no new dependency and no new prompt. Matching is a small allow-list of (bundle id, in-call title) pairs, because "Zoom is open" is not "you are in a call" |
| Auto-*starting* a recording on detection (MTG-11, 2026-09-01) | The title patterns are vendor UI strings that drift, so the first false positive would silently record a room. A privacy-first app does not get to make that mistake by default | Detect and prompt: a dismissable banner offering "Start recording", which runs the same create-then-`autoStart` path as "+ New meeting". The auto-*stop* half is kept, since stopping errs toward a call you restart rather than a room you did not know was taped. MTG-16 (2026-09-09) adds attention, not autonomy, on top of that prompt: a browser `Notification` (once per detected window, gated on permission) and the server raising the installed yogurt app via `open -a yogurt` when a new window shows up with nothing already recording |
