...
- [x] **AUD-6** Find a way to mute just the mic during a live meeting so it's excluded from the transcript, while system audio keeps recording
  ...
  Frontend: `MicMuteToggle` next to `MicDevicePicker` in the toolbar, mounted only while recording. Icon-only (Lucide `Mic`/`MicOff`), strawberry tone + inline "Mic paused" note when muted, matching the app's existing warn-tone pattern.
