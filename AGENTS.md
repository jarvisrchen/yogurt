...
- `.claude/skills/yogurt-control/SKILL.md` operationalizes the common start/stop/check path from that doc as an executable skill.
...
- `just dev` bootstraps the worktree itself (see above) and wants :5173 and :7878, so stop any other instance first - the port guard will otherwise stop and ask. Leave the worktree in place until he has run it; removing the worktree and its merged branch is the last step after the PR lands, not part of opening it.
