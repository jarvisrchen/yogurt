...
- `just land [pr]` is the final step of the task: it waits for CI (skipped for a docs-only change), squash-merges, then removes the worktree, deletes the branch (local and remote), and re-prints the handover section rewritten for `main` - not gated on jarvisrchen having run the manual test first. If he wants to inspect or run the shipped code afterward, it's already on `main`; there is no need to keep the worktree around for that.
